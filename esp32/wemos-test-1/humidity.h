...
double  gramsPerM3(float tempC, float relHumid) {
...
float  calcDewPoint(float tempC, float relHumid) {
  ...
  // Approach #2
  double comp1 = const2*(((const1*tempC)/tempK)+log(relHumid));
  double comp2 = const1-(((const1*tempC)/tempK)+log(relHumid));
  return comp1 / comp2;
...
void printHumidTest() {
  ...
  Serial.printf("X3 RH=%3.3f temp C=%3.3f F=%3.3f AbsHumid=%3.3f ", relHumid, tempC, tempF, absHumid); 
  Serial.printf("OuncePerCubicFoot=%3.7f PoundPerCubicFoot=%3.9f gramPerM3=%3.5f dewPointC=%3.3f\n", ouncePerCubicFoot, poundPerCubicFoot, absHumid,  dewC);
