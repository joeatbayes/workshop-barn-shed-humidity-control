...
http://127.0.0.1:9801/ping?name=Joe&handle=workinghard
...
/* Simple handler to demonstrate responding to
a posted form.   Can be accessed via:
POST http://127.0.0.1:9801/sensorReading
values will print to console. */
func sensorRead(w http.ResponseWriter, r *http.Request) {
	fmt.Println("L43: sensorRead")
	w.Header().Add("content-type", "text/plain")
	...
	//fmt.Println(r.Form) // print form information in server side
	fmt.Println("path", r.URL.Path)
	fmt.Println("reqURI", r.URL.RequestURI())
	fmt.Println("scheme", r.URL.Scheme)
	fmt.Println("query", r.URL.Query())
	fmt.Println("url ", r.URL.RequestURI())
	fmt.Println(r.Form["url_long"])
	fmt.Fprintf(w, "printFormVal\n url=%s\n", r.URL.Path) // send data to client side
	fmt.Println("Body=", string(bodyBuffer))
	for k, v := range r.Form {
		fmt.Println("key:", k, "\t", " val:", strings.Join(v, ""))
		fmt.Fprintf(w, "key:%s\t val=%s\n", k, strings.Join(v, ""))
	}
...
func main() {
	...
	fmt.Println("Listening on port 8091")
	cwd, _ := os.Getwd()
	fmt.Println("cwd=", cwd) // for example /home/user

	http.HandleFunc("/sensorReading/", sensorRead) // set router
	http.HandleFunc("/sensorReading", sensorRead)  // set router
	http.HandleFunc("/ping/", ping)                // set router
	http.HandleFunc("/api/save-cert-of-need/", api_save_cert_of_need)
	// Test:  http://127.0.0.1:8901/api/save-cert-of-need?id=1004
	//  should return a 200

	err := http.ListenAndServe(":8091", nil) // set listen port
	if err != nil {
